...
while ((not gdp.wasCanceled()) and not (gdp.wasOKed())):
	...
	if (input is None):
		...
		cylinderProjectionTransformed_temp = clijx.create([cylinderProjection.getHeight(), cylinderProjection.getDepth(), cylinderProjection.getWidth()], input.getNativeType());
		cylinderProjectionTransformed = clijx.create([cylinderProjection.getDepth(), cylinderProjection.getHeight(), cylinderProjection.getWidth()], input.getNativeType());
		cylinderMaxProjection = clijx.create([cylinderProjectionTransformed.getWidth(), cylinderProjectionTransformed.getHeight()], input.getNativeType());
	...
	if (not stillValid):
		...
		clijx.resliceLeft(cylinderProjection, cylinderProjectionTransformed_temp);
		clijx.rotateRight(cylinderProjectionTransformed_temp, cylinderProjectionTransformed);
		clijx.maximumZProjection(cylinderProjectionTransformed, cylinderMaxProjection);
	...
	if (not stillValid):
		if (doSpotDetection):
			projectionImp = clijx.pull(cylinderMaxProjection);
			...
			if (circles is None):
				circles = clijx.create(cylinderMaxProjection);
			clijx.set(circles, 0);

	# Result visualisation
	if (not stillValid):
		impCylinderProjection = clijx.pull(cylinderMaxProjection);
