...
public class Integer2ToClearCLBufferConverter extends AbstractCLIJConverter<Integer2, ClearCLBuffer> {

    @Override
    public ClearCLBuffer convert(Integer2 source) {
        ...
        for (int y = 0; y < dimensions[1]; y++) {
            for (int x = 0; x < dimensions[0]; x++) {
                inputArray[count] = (int)source.data[x][y];
                count++;
            }
        }
    ...
    @Override
    public Class<Integer2> getSourceType() {
