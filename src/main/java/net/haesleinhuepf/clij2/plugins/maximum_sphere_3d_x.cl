...
__kernel void maximum_sphere_3d
(
  IMAGE_dst_TYPE dst,
  IMAGE_src_TYPE src,
  const int Nx,
...
)
{
  ...
  const int4 coord = (int4){i,j,k,0};
    ...
    float maximumValue = (float)READ_src_IMAGE(src,sampler,coord).x;
  ...
  IMAGE_dst_PIXEL_TYPE res = CONVERT_dst_PIXEL_TYPE(maximumValue);
