...
public class Blur3D extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation {
    ...
    public boolean executeCL() {
        ...
        if (clij2.hasImageSupport()) {
            ...
            blur3D(clij2, image, output, sigmaX, sigmaY, sigmaZ);
            clij2.release(image);
        } else {
            blur3D(clij2, input, output, sigmaX, sigmaY, sigmaZ);
        }
        return true;
    }
    ...
    public static boolean blur3D(CLIJ2 clij2, ClearCLImageInterface src, ClearCLImageInterface dst, Float blurSigmaX, Float blurSigmaY, Float blurSigmaZ) {
        return blur(clij2, src, dst, blurSigmaX, blurSigmaY, blurSigmaZ);
    }
    ...
    public static boolean blur(CLIJ2 clij2, ClearCLImageInterface src, ClearCLImageInterface dst, Float blurSigmaX, Float blurSigmaY, Float blurSigmaZ) {
        return executeSeparableKernel(clij2, src, dst, Blur3D.class, "blur_separable_" + src.getDimension() + "d_x.cl", "blur_separable_" + src.getDimension() + "d", sigmaToKernelSize(blurSigmaX), sigmaToKernelSize(blurSigmaY), sigmaToKernelSize(blurSigmaZ), blurSigmaX, blurSigmaY, blurSigmaZ, src.getDimension());
    }
