 ...
 * January 2021
 */
@Plugin(type = CLIJMacroPlugin.class, name = "CLIJ2_minimumOfTouchingNeighborsMap")
public class MinimumOfTouchingNeighborsMap extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation, IsCategorized, HasClassifiedInputOutput {
    @Override
    ...
    public boolean executeCL() {
        boolean result = minimumOfTouchingNeighborsMap(getCLIJ2(), (ClearCLBuffer) (args[0]), (ClearCLBuffer) (args[1]),  (ClearCLBuffer) (args[2]), asInteger(args[3]), asBoolean(args[4]));
        return result;
    }

    public static boolean minimumOfTouchingNeighborsMap(CLIJ2 clij2, ClearCLBuffer parametric_map, ClearCLBuffer label_map, ClearCLBuffer parametric_map_destination, Integer radius, Boolean ignore_touching_background) {
        if (radius < 0) {
            System.out.println("Warning: Radius < 0 detected in minimumOfTouchingNeighborsMap.");
        }
