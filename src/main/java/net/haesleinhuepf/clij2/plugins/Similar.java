...
public class Similar extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation, IsCategorized, HasClassifiedInputOutput {
    ...
    public boolean executeCL() {
        boolean result = similar(getCLIJ2(), (ClearCLBuffer) (args[0]), (ClearCLBuffer) (args[1]), (ClearCLBuffer) (args[2]), asFloat(args[3]));
        return result;
    }

    public static boolean similar(CLIJ2 clij2, ClearCLBuffer input_image1, ClearCLBuffer input_image2, ClearCLBuffer binary_destination, Float tolerance) {
        ClearCLBuffer temp = clij2.create(input_image1);
        clij2.absoluteDifference(input_image1, input_image2, temp);
        clij2.smallerOrEqualConstant(temp, binary_destination, tolerance);
        temp.close();
        return true;
    }
