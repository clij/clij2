...
public class ExcludeLabelsWithValuesOutOfRange extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation {
    ...
    public boolean executeCL() {
        ...
        return excludeLabelsWithValuesOutOfRange(getCLIJ2(), values, label_map_in, label_map_out, min, max);
    }

    public static boolean excludeLabelsWithValuesOutOfRange(CLIJ2 clij2, ClearCLBuffer values, ClearCLBuffer label_map_in, ClearCLBuffer label_map_out, Float min, Float max) {
        ...
        clij2.smallerConstant(values, below, min);
        clij2.greaterConstant(values, above, max);

        ClearCLBuffer temp = clij2.create(values.getDimensions(), NativeTypeEnum.UnsignedByte);

        clij2.binaryOr(below, above, temp);
