package net.haesleinhuepf.clij.gui;

/**
 * Counter3D adapted from
 * https://github.com/fiji/3D_Objects_Counter/blob/master/src/main/java/Utilities/Counter3D.java
 *
...
public class Counter3D {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (imgArray[currPos] != 0) {
                            ...
                            for (neigbY = y - 1; neigbY <= y + 1; neigbY++) {
                                for (neigbX = x - 1; neigbX <= x + 1; neigbX++) {
                                    ...
                                    if (neigbX >= 0 && neigbX < width && neigbY >= 0 && neigbY < height && neigbZ >= 1 && neigbZ <= nbSlices) {
                                        ...
                                        if (imgArray[pos] != 0) {
                                            if ((nbSlices > 1 && ((neigbX == x && neigbY == y && neigbZ == z - 1) || (neigbX == x && neigbY == y && neigbZ == z + 1))) || (neigbX == x && neigbY == y - 1 && neigbZ == z) || (neigbX == x && neigbY == y + 1 && neigbZ == z) || (neigbX == x - 1 && neigbY == y && neigbZ == z) || (neigbX == x + 1 && neigbY == y && neigbZ == z))
                                                surfPix++;
                        ...
                        if (x == 0 || y == 0 || x == width - 1 || y == height - 1 || (nbSlices != 1 && (z == 1 || z == nbSlices)))
                            IDisAtEdge[minID] = true;
                    }
                    currPos++;
                }
            }
    ...
    public void setObjects(int[] objID, int[] IDcount) {
        this.objID = objID;
        this.IDcount = IDcount;

        nbObj = IDcount.length;
        foundObjects=true;
    }
     ...
     * @param newVal value to be replaced by. </P>
