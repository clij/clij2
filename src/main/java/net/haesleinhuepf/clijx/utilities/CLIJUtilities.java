...
public class CLIJUtilities extends net.haesleinhuepf.clij.utilities.CLIJUtilities {
    ...
    public static boolean executeSeparableKernel(CLIJx clijx, ClearCLImageInterface src, ClearCLImageInterface dst, Class anchorClass, String clFilename, String kernelname, int kernelSizeX, int kernelSizeY, int kernelSizeZ, float blurSigmaX, float blurSigmaY, float blurSigmaZ, long dimensions) {
        ...
        if (src instanceof ClearCLBuffer) {
            temp1 = clijx.create(((ClearCLBuffer) src).getDimensions(), NativeTypeEnum.Float);
            temp2 = clijx.create(((ClearCLBuffer) src).getDimensions(), NativeTypeEnum.Float);
        } else if (src instanceof ClearCLImage) {
            temp1 = clijx.create(((ClearCLImage) src).getDimensions(), ImageChannelDataType.Float);
            temp2 = clijx.create(((ClearCLImage) src).getDimensions(), ImageChannelDataType.Float);
        } else {
        ...
        if (blurSigma[0] > 0) {
            ...
            clijx.execute(anchorClass, clFilename, kernelname, src.getDimensions(), src.getDimensions(), parameters);
        } else {
            if (dimensions == 2) {
                clijx.copy(src, temp1);
            } else {
                clijx.copy(src, temp2);
            }
        }
