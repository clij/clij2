...
__kernel void generate_distance_matrix(
  ...
  for (int i = 0; i < n_dimensions; i ++) {
      int2 pos = (int2){dx, i};
      positions[i] = READ_src_point_list1_IMAGE(src_point_list1, sampler, pos).x;
  }
  ...
  for (int j = 0; j < GET_IMAGE_WIDTH(src_point_list2); j ++) {
      ...
      for (int i = 0; i < n_dimensions; i ++) {
          int2 pos = (int2){j, i};
          sum = sum + pow(positions[i] - (float)READ_src_point_list2_IMAGE(src_point_list2, sampler, pos).x, (float)2);
      }
      float out = sqrt(sum);
