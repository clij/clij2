 ...
 * December 2018
 */
@Plugin(type = CLIJMacroPlugin.class, name = "CLIJx_copySlice")
public class CopySlice extends AbstractCLIJxPlugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation {

    @Override
    public boolean executeCL() {
        return copySlice(getCLIJx(), (ClearCLBuffer)( args[0]), (ClearCLBuffer)(args[1]), asInteger(args[2]));
    }

    public static boolean copySlice(CLIJx clijx, ClearCLImageInterface src, ClearCLImageInterface dst, Integer planeIndex) {
        ...
        if (src.getDimension() == 2 && dst.getDimension() == 3) {
            clijx.execute(CopySlice.class, "copy_slice_to_3d_x.cl", "copy_slice_to_3d", src.getDimensions(), src.getDimensions(), parameters);
        } else if (src.getDimension() == 3 && dst.getDimension() == 2) {
            clijx.execute(CopySlice.class, "copy_slice_from_3d_x.cl", "copy_slice_from_3d", dst.getDimensions(), dst.getDimensions(), parameters);
        } else {
