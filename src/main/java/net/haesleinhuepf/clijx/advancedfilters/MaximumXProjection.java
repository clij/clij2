...
public class MaximumXProjection extends AbstractCLIJxPlugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation {
    ...
    public static boolean maximumXProjection(CLIJx clijx, ClearCLBuffer src, ClearCLBuffer dst_max) {
        ...
        clijx.execute(MaximumXProjection.class, "maximum_x_projection_" + src.getDimension() + "d_" + dst_max.getDimension() + "d_x.cl", "maximum_x_projection_" + src.getDimension() + "d_" + dst_max.getDimension() + "d", dst_max.getDimensions(), dst_max.getDimensions(), parameters);
        return true;
    }
