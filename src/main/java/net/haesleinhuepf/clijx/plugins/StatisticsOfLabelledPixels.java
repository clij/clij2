...
@Plugin(type = CLIJMacroPlugin.class, name = "CLIJx_statisticsOfLabelledPixels")
public class StatisticsOfLabelledPixels extends AbstractCLIJPlugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation {
    ...
    public static double[][] statisticsOfLabelledPixels(CLIJ clij, ClearCLBuffer inputImage, ClearCLBuffer inputLabelMap) {
        ...
        return statisticsOfLabelledPixels(clij, inputImage, inputLabelMap, 1, numberOfLabels);
    }

    public static double[] statisticsOfLabelledPixels(CLIJ clij, ClearCLBuffer inputImage, ClearCLBuffer inputLabelMap, int labelIndex) {
        return statisticsOfLabelledPixels(clij, inputImage, inputLabelMap, labelIndex, labelIndex)[0];
    }
    ...
    public static double[][] statisticsOfLabelledPixels(CLIJ clij, ClearCLBuffer inputImage, ClearCLBuffer inputLabelMap, int startLabelIndex, int endLabelIndex) {
        ...
        if (inputImage.getNativeType() != NativeTypeEnum.Float) {
            image = clij.create(inputImage.getDimensions(), NativeTypeEnum.Float);
            clij.op().copy(inputImage, image);
        }

        if (inputLabelMap.getNativeType() != NativeTypeEnum.Float) {
            labelMap = clij.create(inputLabelMap.getDimensions(), NativeTypeEnum.Float);
            clij.op().copy(inputLabelMap, labelMap);
        }

        ImagePlus imp = clij.pull(image);
        ImagePlus lab = clij.pull(labelMap);
