...
__kernel void median_slice_by_slice_box_3d
(
  IMAGE_dst_TYPE dst,
  IMAGE_src_TYPE src,
  const int Nx,
...
)
{
  ...
  for (int x = -e.x; x <= e.x; x++) {
    ...
    for (int y = -e.y; y <= e.y; y++) {
      array[count] = (IMAGE_dst_PIXEL_TYPE)READ_src_IMAGE(src,sampler,coord+((int4){x,y,0,0})).x;
      count++;
    }
  }
  array_size = count;
  //copyBoxSliceNeighborhoodToArray(src, array, coord, Nx, Ny);
