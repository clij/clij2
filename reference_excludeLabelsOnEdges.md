...
clijx.excludeLabelsOnEdges(clij, label_map_input, label_map_destination);
```

```
//show result
label_map_destinationImagePlus = clij.pull(label_map_destination);
label_map_destinationImagePlus.show();

// cleanup memory on GPU
label_map_input.close();
label_map_destination.close();
```
